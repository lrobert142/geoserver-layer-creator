...
public class GeoserverDataUpdater {
	...
	public boolean updateAllData(GeoServerRESTReader reader,
			GeoServerRESTPublisher publisher, List<ShapeFile> dataSets,
			String csvFileName) {
		...
		for (ShapeFile dataSet : dataSets) {
			...
			if (layer.getType() == RESTLayer.Type.VECTOR) {
				feature = reader.getFeatureType(reader.getLayer(
						dataSet.getWorkspace(), dataSet.getLayerName()));
				successful = updater.updateFeature(publisher, dataSet,
						feature.getNativeCRS());
			} else {
		...
		new GeoserverToCsvWriter()
				.writeDataSetsToCsvFile(dataSets, csvFileName);
		return successful;
	}
	...
	protected boolean updateFeature(GeoServerRESTPublisher publisher,
			ShapeFile dataSet, String nativeCRS)
			throws IllegalArgumentException {
		if (nativeCRS == null || nativeCRS.isEmpty())
			throw new IllegalArgumentException(
					"nativeCRS cannot be null or empty.");
		boolean successful = true;
		...
		String title = dataSet.getTitle() == null
				|| dataSet.getTitle().isEmpty() ? "No_title_provided" : dataSet
				.getTitle();
		String layerAbstract = dataSet.getLayerAbstract() == null
				|| dataSet.getLayerAbstract().isEmpty() ? "No abstract provided"
		...
		for (String metaDataLink : dataSet.getMetadataXmlHref().split(",")) {
			featureEnc.addMetadataLinkInfo("text/xml", "TC211",
					metaDataLink.trim());
		}
		...
		if (publisher.unpublishFeatureType(workspace, dataStoreName, layerName)) {
			if (!publisher.publishDBLayer(workspace, dataStoreName, featureEnc,
					featureLayerEnc)) {
				logger.error("Feature " + layerName
						+ " was unpublished but could not be modified!");
				successful = false;
			} else {
				logger.info("Feature " + layerName
	...
	protected boolean updateCoverage(GeoServerRESTPublisher publisher,
		...
		for (String metaDataLink : dataSet.getMetadataXmlHref().split(",")) {
			coverageEnc.addMetadataLinkInfo("text/xml", "TC211",
					metaDataLink.trim());
		}
		...
		if (publisher.configureCoverage(coverageEnc, workspace, dataStore)
				&& publisher.configureLayer(workspace, coverageName,
						coverageLayerEnc)) {
			logger.info("Coverage " + coverageName
					+ " was successfully updated!");
		} else {
			logger.error("Coverage " + coverageName
					+ " was unpublished but could not be modified!");
			successful = false;
		}
		return successful;
