...
public class GeoserverToCsvWriter {
	...
	public boolean storeLayerDataInCsv(List<List<String>> dataSets,
			GeoserverDataReader dataRetriever, String fileName)
			throws MalformedURLException {
		String workspace;
		String layerName;
		String layerType;
		...
		for (List<String> layerData : dataSets) {
			...
			if (layerType == "VECTOR")
				layerDataSets.add(dataRetriever.getLayerDataFromGeoserver(
						workspace, layerName));
			else
				layerDataSets.add(dataRetriever.getCoverageDataFromGeoserver(
						workspace, layerName));
		}
	 ...
	 *         <code>false</code> otherwise.
	 */
	protected boolean writeDataSetsToCsvFile(List<ShapeFile> dataSets,
		...
		try {
			...
			dataToWrite.add(new String[] { "storePath", "BASENAME",
					"storeName", "layerName", "workspace", "storeType",
					"title", "abstract", "metadataXmlHref", "keywords",
					"wmsPath", "styles", "uploadData", "uploadMetadata" });
