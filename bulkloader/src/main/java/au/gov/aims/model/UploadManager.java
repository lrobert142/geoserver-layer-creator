...
public class UploadManager {
	...
	public UploadManager() {
		logger = Logger.getLogger(UploadManager.class);
	...
	public boolean uploadGeoServerFilesToGeoServer(String csvFileName) {
		if(geoServerManager.checkConnectionExists()) {
			currentIndex = 0;
	...
	/**
	 * Gets the index of the list of uploads that the geoserver file that is currently uploading from
	 * @return int - the index of the list of uploads that the geoserver file that is currently uploading from
	 */
	...
	/**
	 * Checks if the upload is completed or not
	 * @return boolean - true if the upload has finished, false if it has not
	 */
	...
	/**
	 * Gets the store path of the geoserver file at the index specified in the list of files to upload
	 * @param index - the index of the geoserver file in the list of uploads
	 * @return String - the store path of the geoserver file at the index in the list of uploads
	 */
	public String getUploadAt(int index) {
	...
	public void stopUpload(String directoryPath) {
		if(!stopUpload) {
			stopUpload = true;
			if(geoServerFiles != null) {
				for(int i = currentIndex; i < geoServerFiles.size(); ++i) {
